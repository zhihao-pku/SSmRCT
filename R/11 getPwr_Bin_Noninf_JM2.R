...
#'     pc_i = c(0.6, 0.6),
#'     fi = c(f, 1 - f), cut = 0.3,
#'     alpha = 0.025, N = 100, r = 1, direct = 1, sim = F
#'   )$overall
#'   res$M <- "calc"
#'   res$f <- f
#'   res
...
#'     pc_i = c(0.6, 0.6),
#'     fi = c(f, 1 - f), cut = 0.3,
#'     alpha = 0.025, N = 100, r = 1, direct = 1, sim = T
#'   )$overall
#'   res$M <- "sim"
#'   res$f <- f
#'   res
...
getPwr_Bin_Noninf_JM2 <- function(pt_i, pc_i, fi, cut, alpha, N, r, direct = 1, sim = F, nsim = 1000, seed = 0) {
  if (!sim) {
    Ni <- N * fi
    gr <- 2 + r + 1 / r
    delta_i <- pt_i - pc_i
