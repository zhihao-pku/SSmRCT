...
#'   pi = 0.5, alpha = 0.025, N = 400, r = 1, sim = F
#' )
#' dat1$M <- "calc"
#' dat2 <- getPwr_Surv_Equi_JM1(
#'   delta_j = log(1.1), delta_nj = log(1.0),
#'   f = seq(0.1, 0.9, 0.1), cut = log(1.3),
#'   pi = 0.5, alpha = 0.025, N = 400, r = 1, sim = T
#' )
#' dat2$M <- "sim"
#'
...
getPwr_Surv_Equi_JM1 <- function(delta_j, delta_nj, f, pi, cut, alpha, N, r, lambda0_j = 1, lambda0_nj = 1, sim = F, nsim = 1000, seed = 0) {
  if (!sim) {
  ...
  if (sim) {
    ...
    ss <- map_dfr(.x = 1:nsim, .f = function(i) {
      ...
      succ_a <- if_else(za_u < cut & za_l > (-cut), 1, 0)
