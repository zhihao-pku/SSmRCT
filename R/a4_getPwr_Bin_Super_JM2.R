...
#' @name getPwr_Bin_JM2
#'
#' @param p1_i A vector. Rate of treatment group in each region.
#' @param p0_i A vector. Rate of control group in each region.
...
#'   }
#' )
getPwr_Bin_Super_JM2 <- function(p1_i, p0_i, fi, alpha = 0.025, beta = NA, N = NA, r = 1, scale = "RD", sim = FALSE, nsim = 1000, seed = 0) {
  if (!sim) {
    num <- length(fi)
    p1_a <- sum(p1_i * fi)
    p0_a <- sum(p0_i * fi)
  ...
  if (sim) {
    ...
    res <- suppressMessages({
      da %>%
        summarise(pwr1 = mean(succ_a), pwr2 = mean(succ_i), pwr3 = mean(succ_a & succ_i), pwr4 = mean(succ_i[succ_a == 1])) %>%
...
#' @rdname getPwr_Bin_JM2
#' @export
getPwr_Bin_Noninf_JM2 <- function(p1_i, p0_i, fi, cut, alpha = 0.025, beta = NA, N = NA, r = 1, scale = "RD", direct = 1, sim = FALSE, nsim = 1000, seed = 0) {
  if (!sim) {
    num <- length(fi)
    p1_a <- sum(p1_i * fi)
    p0_a <- sum(p0_i * fi)
    ...
    u <- if_else(direct == 1, (delta_a + cut) / sqrt(var_a), (delta_a - cut) / sqrt(var_a))
    ui <- if_else(direct == rep(1, num), (delta_i + cut) / sqrt(var_i), (delta_i - cut) / sqrt(var_i))
    M <- diag(num + 1)
    M[1, ] <- c(1, sqrt(fi))
    M[, 1] <- c(1, sqrt(fi))
    if (direct == -1) {
  ...
  if (sim) {
    ...
    for (j in 1:nsim) {
      ...
      zi <- if_else(direct == rep(1, num), delta_i_est + cut, delta_i_est - cut)
...
#' @rdname getPwr_Bin_JM2
#' @export
getPwr_Bin_Equi_JM2 <- function(p1_i, p0_i, fi, cut, alpha = 0.025, beta = NA, N = NA, r = 1, scale = "RD", sim = FALSE, nsim = 1000, seed = 0) {
  if (!sim) {
    num <- length(fi)
    p1_a <- sum(p1_i * fi)
    p0_a <- sum(p0_i * fi)
    ...
    ui1 <- (delta_i + cut) / sqrt(var_i)
    ui2 <- (delta_i - cut) / sqrt(var_i)
    M <- diag(num + 1)
    M[1, ] <- c(1, sqrt(fi))
    M[, 1] <- c(1, sqrt(fi))
    M1 <- combine(M, M)
