...
#'
#' @param delta_i A vector. log(HR) between treatment and control groups for each region.
...
#' \deqn{1 - e^{\hat \delta_i} < 0 \text{ for i = 1, 2, .., m}}
#'
...
#' \deqn{\frac{1}{e^{\Delta}} - e^{\hat \delta_i} < 0 \text{ for i = 1, 2, .., m}}
#'
...
#' \deqn{\frac{1}{e^{\Delta}} - e^{\hat \delta_i} < 0\text{ and }e^{\Delta} - e^{\hat \delta_i} > 0 \text{ for i = 1, 2, .., m}}
#'
...
#'       ),
#'       fi = c(f, 1 - f),
#'       alpha = 0.025, beta = NA, N = 300, r = 1, criterion = 1, sim = FALSE
#'     )$overall
#'     res$f <- f
#'     res
#'   }
...
#'   }
#' )
getPwr_Surv_Super_JM2 <- function(delta_i, fi, alpha = 0.025, beta = NA, N = NA, r = 1, criterion = 1, sim = FALSE, nsim = 1000, seed = 0) {
  if (!sim) {
    num <- length(delta_i)
    ...
    if (criterion == 2) {
      ...
      u <- -(1 - exp(delta_a)) / se
      sei <- sqrt(gr / Ni * exp(delta_i)^2)
      ui <- -(1 - exp(delta_i)) / sei
      cov <- gr / N * exp(delta_i) * exp(delta_a)
      corr <- cov / (sei * se)
    }
    M <- diag(num + 1)
    M[1, ] <- c(1, corr)
...
#' @rdname getPwr_Surv_JM2
#' @export
getPwr_Surv_Noninf_JM2 <- function(delta_i, fi, cut, alpha = 0.025, beta = NA, N = NA, r = 1, criterion = 1, direct = 1, sim = FALSE, nsim = 1000, seed = 0) {
  if (!sim) {
    num <- length(delta_i)
    delta_a <- sum(delta_i * fi)
    if (is.na(N) & (!is.na(alpha)) & (!is.na(beta))) {
    ...
    if (criterion == 2) {
      ...
      sei <- sqrt(gr / Ni * exp(delta_i)^2)
      ui <- if_else(direct == rep(1, num), -(1 / exp(cut) - exp(delta_i)) / sei, -(exp(cut) - exp(delta_i)) / sei)
      cov <- gr / N * exp(delta_i) * exp(delta_a)
      corr <- cov / (sei * se)
    }
    M <- diag(num + 1)
...
#' @rdname getPwr_Surv_JM2
#' @export
getPwr_Surv_Equi_JM2 <- function(delta_i, fi, cut, alpha = 0.025, beta = NA, N = NA, r = 1, criterion = 1, sim = FALSE, nsim = 1000, seed = 0) {
  if (!sim) {
    num <- length(delta_i)
    delta_a <- sum(delta_i * fi)
    if (is.na(N) & (!is.na(alpha)) & (!is.na(beta))) {
    ...
    if (criterion == 2) {
      ...
      sei <- sqrt(gr / Ni * exp(delta_i)^2)
      ui1 <- -(1 / exp(cut) - exp(delta_i)) / sei
      ui2 <- -(exp(cut) - exp(delta_i)) / sei
      cov <- gr / N * exp(delta_i) * exp(delta_a)
      corr <- cov / (sei * se)
    }
    M <- diag(num + 1)
    ...
    L <- list(overall = res, pwr_margin = pwr_margin, p_condition = p_condition, pwr_joint = pwr_joint)
  }
  if (sim) {
    num <- length(delta_i)
    delta_a <- sum(delta_i * fi)
