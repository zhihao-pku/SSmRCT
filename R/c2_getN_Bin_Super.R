...
#'
#' Calculating sample size when given power or power when given sample size for binary endpoints.
#'
#' @rdname getN_Bin
#'
#' @name getN_Bin
#'
#' @param p1 Rate of treatment group.
#' @param p0 Rate of control group.
...
#'
#' @details
#' Taking the larger RD/RR/OR is preferable as an example. Sample size calculation is based on the following Z test, with the success criterion:
#'
...
#'
#' Where \eqn{\hat \delta = \hat p_1 - \hat p_0, \hat \sigma_0^2 = \hat p_0(1 - \hat p_0), \hat \sigma_1^2 = \hat p_1(1 - \hat p_1)} for RD; \eqn{\hat \delta = log(\frac{\hat p_1}{\hat p_0}), \hat \sigma_0^2 = \frac{(1 - \hat p_0)}{\hat p_0}, \hat \sigma_1^2 = \frac{(1 - \hat p_1)}{\hat p_1}} for RR; \eqn{\hat \delta = log(\frac{\hat p_1 / (1 - \hat p_1)}{\hat p_0 / (1 - \hat p_0)}, \hat \sigma_0^2 = \frac{1}{\hat p_0(1 - \hat p_0)}), \hat \sigma_1^2 = \frac{1}{\hat p_1(1 - \hat p_1)}} for OR; and \eqn{\Delta} is the non-inferiority or equivalence margin.
#'
#' @export
#'
#' @examples
...
getN_Bin_Super <- function(p1, p0, alpha = 0.025, beta = NA, N = NA, r = 1, scale = "RD") {
  ...
  res <- future_map_dfr(.x = 1:nrow(eg), .f = function(i) {
    ...
    if (is.na(N) & (!is.na(beta))) {
      ...
      df <- data.frame(p1, p0, scale, delta, alpha, beta, pwr, r, N, n1, n0)
    }
    if (is.na(beta) & (!is.na(N))) {
      if (scale == "RD") {
        delta <- p1 - p0
...
getN_Bin_Noninf <- function(p1, p0, cut, alpha = 0.025, beta = NA, N = NA, r = 1, scale = "RD", direct = 1) {
  ...
  res <- future_map_dfr(.x = 1:nrow(eg), .f = function(i) {
    ...
    getPwr <- function(n0, delta, sigma1, sigma0) {
      n1 <- r * n0
      z <- if_else(direct == 1, (delta + cut) / sqrt(sigma1^2 / n1 + sigma0^2 / n0), (delta - cut) / sqrt(sigma1^2 / n1 + sigma0^2 / n0))
    ...
    if (is.na(N) & (!is.na(beta))) {
      ...
      n0 <- if_else(direct == 1, (qnorm(1 - alpha) + qnorm(1 - beta))^2 * (sigma1^2 + sigma0^2 / r) / (delta + cut)^2, (qnorm(1 - alpha) + qnorm(1 - beta))^2 * (sigma1^2 + sigma0^2 / r) / (delta - cut)^2)
      n0 <- ceiling(n0)
      n1 <- r * n0
      N <- n1 + n0
      pwr <- getPwr(n0, delta, sigma1, sigma0)
      df <- data.frame(p1, p0, cut, scale, delta, alpha, beta, pwr, r, N, n1, n0, direct)
    }
    if (is.na(beta) & (!is.na(N))) {
      if (scale == "RD") {
        delta <- p1 - p0
...
getN_Bin_Equi <- function(p1, p0, cut, alpha = 0.025, beta = NA, N = NA, r = 1, scale = "RD", maxN = 1e+06) {
  ...
  res <- future_map_dfr(.x = 1:nrow(eg), .f = function(i) {
    ...
    if (is.na(N) & (!is.na(beta))) {
      ...
      N <- n1 + n0
      pwr <- getPwr(n0, delta, sigma1, sigma0)
      df <- data.frame(p1, p0, cut, scale, delta, alpha, beta, pwr, r, N, n1, n0)
    }
    if (is.na(beta) & (!is.na(N))) {
      if (scale == "RD") {
        delta <- p1 - p0
