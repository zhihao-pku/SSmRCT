...
#' @name getN_Count
#'
#' @param delta log(RR) between treatment and control groups.
#' @param lambda0 Baseline hazard of control group.
...
getN_Count_Super <- function(delta, lambda0, t, k = 0, alpha = 0.025, beta = NA, N = NA, r = 1) {
  ...
  res <- future_map_dfr(.x = 1:nrow(eg), .f = function(i) {
    ...
    if (is.na(N) & (!is.na(beta))) {
      ...
      N <- n1 + n0
      pwr <- getPwr(n0)
      df <- data.frame(delta, lambda0, t, k, alpha, beta, pwr, r, N, n1, n0)
    }
    if (is.na(beta) & (!is.na(N))) {
      n0 <- N / (r + 1)
      n1 <- r * n0
...
getN_Count_Noninf <- function(delta, lambda0, t, k = 0, cut, alpha = 0.025, beta = NA, N = NA, r = 1, direct = 1) {
  ...
  res <- future_map_dfr(.x = 1:nrow(eg), .f = function(i) {
    ...
    if (is.na(N) & (!is.na(beta))) {
      ...
      N <- n1 + n0
      pwr <- getPwr(n0)
      df <- data.frame(delta, lambda0, t, k, cut, alpha, beta, pwr, r, N, n1, n0, direct)
    }
    if (is.na(beta) & (!is.na(N))) {
      n0 <- N / (r + 1)
      n1 <- r * n0
...
getN_Count_Equi <- function(delta, lambda0, t, k = 0, cut, alpha = 0.025, beta = NA, N = NA, r = 1, maxN = 1e+06) {
  ...
  res <- future_map_dfr(.x = 1:nrow(eg), .f = function(i) {
    ...
    if (is.na(N) & (!is.na(beta))) {
      ...
      N <- n1 + n0
      pwr <- getPwr(n0)
      df <- data.frame(delta, lambda0, t, k, cut, alpha, beta, pwr, r, N, n1, n0)
    }
    if (is.na(beta) & (!is.na(N))) {
      n0 <- N / (r + 1)
      n1 <- r * n0
